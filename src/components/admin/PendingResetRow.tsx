...
const initialState: ApproveResetState = {};
...
export default function PendingResetRow({
  ...
  const [state, formAction, pending] = useActionState(approvePasswordResetAction, initialState);

  if (state.tempPassword) {
    return (
      <div className="rounded-xl border border-green-200 bg-green-50 p-4">
        <p className="text-sm text-green-800">
          Nouveau mot de passe temporaire pour <strong>{state.adminName}</strong> :
        </p>
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-brand/30 bg-brand/5 p-4">
      ...
      <div className="flex items-center gap-2">
        <form action={rejectPasswordResetAction}>
